document.addEventListener('DOMContentLoaded', () => {
  ...
  filterButtons.forEach(button => {
    button.addEventListener('click', function () {
      ...
      this.classList.add('bg-neutral-200', 'text-neutral-900');

      const severity = this.textContent.trim().toLowerCase();
...
async function loadReport() {
  try {
    ...
    if (vulnGrid) {
      ...
      const severityColors = {
        critical: {
          ...
          mark: 'text-red-600 hover:text-red-700'
        },
        high: {
          badge: 'text-orange-700 bg-orange-100',
          mark: 'text-orange-600 hover:text-orange-700'
        },
        medium: {
          badge: 'text-yellow-700 bg-yellow-100',
          mark: 'text-yellow-600 hover:text-yellow-700'
        },
        low: {
      ...
      report.detailed_findings.vulnerabilities.forEach(vuln => {
        ...
        const colors = severityColors[severity] || severityColors.low; // Default to low if severity is unknown

        const vulnCard = `
          <div class="border border-neutral-200/20 rounded-lg p-4 hover:bg-neutral-50 vuln-card ${severity}">
            <div class="flex justify-between items-start mb-3">
              <span class="px-2 py-1 text-xs font-semibold ${colors.badge} rounded-full">${vuln.severity}</span>
              <span class="text-sm text-neutral-500">${new Date(vuln.timestamp).toLocaleDateString()}</span>
            </div>
            ...
            <div class="flex justify-end space-x-3">
              <button class="flex items-center px-3 py-1 text-sm font-medium text-neutral-700 hover:text-neutral-900">
              ...
              <button class="flex items-center px-3 py-1 text-sm font-medium ${colors.mark}">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    setupPagination(report.detailed_findings.vulnerabilities.length);
...
function setupPagination(totalItems) {
  ...
  const previousButton = document.querySelector('.px-3.py-1.border.border-neutral-200/20.rounded.disabled\\:opacity-50');
  const nextButton = document.querySelector('.px-3.py-1.border.border-neutral-200/20.rounded.hover\\:bg-neutral-50');
  ...
  function updatePaginationDisplay() {
    ...
    const pageInfo = document.querySelector('.text-sm.text-neutral-600');
    if (pageInfo) {
  ...
  itemsPerPageSelect.addEventListener('change', () => {
    loadPage(1, parseInt(itemsPerPageSelect.value, 10)); // Reset to first page
  });
...
// Function to load a specific page of vulnerabilities
